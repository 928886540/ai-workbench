...
async function loadSettings(refresh = false): Promise<void> {
  ...
  loading.value = true;
  status.value = refresh ? "正在刷新模型目录…" : "正在加载模型设置…";
  statusTone.value = "neutral";
  try {
    applySettings(await api.getModelSettings(props.sessionId, refresh));
    status.value = catalogError.value
      ? `目录拉取失败，仍可手动输入：${catalogError.value}`
      : `${models.value.length} 个可用模型，点击条目即可选择`;
...
<template>
  <section class="page-panel settings-panel" aria-labelledby="settings-title">
    ...
    <article class="settings-card">
      ...
      <p v-if="baseUrl" class="settings-endpoint">{{ baseUrl }}</p>
      ...
      <div class="settings-row">
        <input
          ...
          autocomplete="off"
          spellcheck="false"
          :placeholder="`跟随默认 · ${defaultModel || '未加载'}`"
          aria-label="模型 ID"
