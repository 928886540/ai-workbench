...
def test_error_bubble_keeps_summary_visible_and_details_collapsed() -> None:
    ...
    error_region = bubble.split(
        'v-if="displayedMessage.status === \'error\'"', 1
    )[1].split('v-else-if="displayedMessage.status === \'cancelled\'"', 1)[0]
    for fragment in (
...
def test_error_retry_toolbar_and_plain_text_styling_are_preserved() -> None:
    ...
    assert '["done", "error", "cancelled"].includes(displayedMessage.value.status)' in bubble
